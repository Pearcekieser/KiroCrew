...
| `ACP_BACKENDS_MODEL_EFFORT_PAIR_IDS` | The harness's legacy `models` list ADVERTISES `<model>[<effort>]` pairs its `model` option does not accept whole. For a member the `model` select is therefore the captured model list (`session_models_envelope`, falling back to the pairs only when the select is empty), so the picker shows one row per model beside the effort control. A stored pair pin is a literal miss against the bare list, and `resolve_pin_spelling` (handed the backend) resolves the pair's base and returns the pair, so the session keeps the pinned model AND its effort: the exhausted spelling ladder on the handle falls through to two writes (bare model, then the effort option), and the recorded model id stays the pair so per-model effort overrides keep matching. A pair that reaches `set_model` directly takes the same two writes. A non-member's refused bracketed id stays refused: claude's `[1m]` is a context WINDOW that must reach the wire intact, and opencode's `provider/model` ids carry no suffix at all, so neither may inherit a split it never advertised. Membership also gates the "adapter mismatch, not an account restriction" wording in `AcpModelUnavailable`, because "advertised implies entitled" is established only for a harness whose advertised list IS its entitlement. |
