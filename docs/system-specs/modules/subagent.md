...
The conversation remains busy until that cleanup finishes, so a
continuation cannot reseed the registry while release is deleting the same
session files. A
...
local terminal callback and is left to fenced durable recovery. This prevents a
late commit from producing both an immediate unfenced failure and a later
...
only after the rejected command outcome commits. Cancelling a keyed queued batch
member follows the same settle-then-announce ordering, so its terminal event can
close a wave and release already-held sibling results. If the manager has already
returned a known batch rejection but its settlement outcome is uncertain, the
authority still announces that terminal member before surfacing uncertainty; the
...
accounting closes even though there is no command fence to settle.
...
any execution-time renewal. If orderly-shutdown settlement fails, authority
...
response was lost. Only a second uncertain result leaves the record waiting and
nonterminal so cancellation can retry the retained claim instead of announcing
a false failure.
...
and held-sibling settlement debt. Keyed rejections already have an admitted run,
so they retain its fence and use the normal terminal commit to create the same
pending outbox event after command settlement.
...
3. A recorded child is killed only when the active fenced executor persisted
   ...
   An unverified, unreadable, or missing identity is never signalled and leaves the
   coordinator run nonterminal for a later fenced recovery attempt. A termination
   error likewise leaves the run nonterminal.
