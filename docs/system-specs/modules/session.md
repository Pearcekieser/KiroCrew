...
| Cron / TaskRunner / hook / channel-agent | **unbound** — unattended turns publish the pid sidecar only and clear any leftover human principal. `channel:` is `run_channel_agent`, not a human ChannelTurn. |
...
peeks and does not
`take()` the ContextVar. After the same-key winner holds the
per-session semaphore it applies again (existing-session path,
creator after registration acquire, won-race loser after
reacquire) and recycles if the fingerprint changed. Apply runs
after that `finally` because the lock is not reentrant and
apply may recurse. The staged ContextVar is not consumed before
that acquire, so two Discord users posting into a new thread
cannot overwrite one sidecar.
...
(`cron:`, `taskrunner:`, `subagent:`, `hook:`, `channel:`, `wf:` /
