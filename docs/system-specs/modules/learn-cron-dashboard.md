...
- **Settings → Voice** (`/settings/voice`) — the Speech-to-Text card (`website/src/pages/settings/SttSettings.tsx`, rendered inside `VoicePanel.tsx`, so voice-in and voice-out settings share one tab). Controls: enabled, provider (`local` / `apple` / `transcribe`), model, language, and the dictation knobs `streaming` / `silence_ms` / `partial_interval_ms` / `idle_evict_secs` / `endpointing` / `dictation_panel`. **Nothing in the offered set is hardcoded in the frontend**: `providers`, `models`, `language_codes` and `streaming_providers` all come from `GET /api/config/stt`, because what a host can run differs per host (`_stt_providers()` omits `apple` entirely where macOS is too old or the Swift toolchain is absent, rather than offering an option that cannot be selected) and it is the single source of truth for what GET advertises AND what PUT accepts. `streaming_providers` is served from `stt_stream._STREAMING_PROVIDERS` so the streaming controls gate on a CAPABILITY rather than on a provider name; a hardcoded copy goes stale in the permissive direction, offering a provider this host cannot run or hiding a control for one it can. PUT applies **floors only** on the three millisecond/second knobs, each floor owned by the module that reads the value (`vad.MIN_SILENCE_MS`), because a value above the detector's own cap is inert rather than unsafe; a `bool` is rejected explicitly since it subclasses `int` and a checkbox value would otherwise persist as `1`.
  ...
  - **There is no install button.** No provider needs a runtime installed out of band: the recognizer ships in the optional `voice` extra, `local` fetches its own model and `apple` compiles its own helper on demand. `prereqs` is therefore at most two commands, the `voice` extra in the gateway's OWN interpreter (both halves of that extra are imported in-process, so a system python or a `--user` install cannot satisfy it) plus a platform ffmpeg command, and an empty list is the steady state. The pip command is suppressed where no channel into this interpreter exists (frozen build, code-signed app bundle, pip-less or PEP 668 externally-managed python): `transcribe_unsupported` makes the page show an unsupported notice instead of a command that cannot succeed, and `bundled_interpreter` discriminates the desktop app, whose remedy is a pip install of Kiro Crew rather than "run the gateway from a different Python environment", which is not actionable inside an app bundle. A platform with no prebuilt wheel is not pip-actionable either; the availability `detail` on `/api/stt/status` is what names it. `ffmpeg_missing` is served independently of `available`, because a provider can read ready while an uploaded WebM still cannot be decoded. `transcribe` additionally renders `AwsConsentGate service="transcribe"`, which is the only way the paid-service grant is recorded (deliberately no CLI verb).
...
runs Python in a fork child. GitLab
...
and falls back to the legacy `merge_status` field for older self-managed instances.
...
unlimited. Dashboard creation is create-only under the service lock unless it
names the exact terminal monitor being replaced. That terminal replacement is
conditional on the id and configuration generation read by the request; active,
missing, and stale records return 409 without replacing their evidence. An
in-progress dashboard slot close also returns 409 under the service-lock admission
check; the close marker is tied to the exact in-memory slot generation and is cleared
when the close rolls back, so a terminal replacement cannot outlive its session. An
ordinary create returns 409 if any automation already occupies the slot, even
when that record was armed after the dashboard's last read.
...
three-provider-error defaults. Terminal evidence remains visible with separate
restart and new-monitor actions. The new-monitor action opens a fresh form and
submits the retained terminal id, so changing source providers cannot replace a
concurrent restart or active monitor. When an active monitor's target changes
providers, a successful stop keeps the validated replacement draft open and
carries its target and tuned budgets into the terminal replacement form even when
the terminal WebSocket frame arrives before the stop response. The form
