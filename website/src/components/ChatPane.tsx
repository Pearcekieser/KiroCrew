...
export default function ChatPane({
  ...
  const doSend = useCallback(() => {
    ...
    // the payload back; `unknown` and `response-late` both mean the request
    // WAS received and only the answer is missing — the output arrives over
    // the socket, so restoring would invite a retry that duplicates a turn
    // already in flight, side effects included.
    void sendTurn({ message: llm, slot: slotKey, meta }).then((receipt) => {
      ...
      // (#4131) — no `chat_message` echo is coming for a dashboard send. Only
      // an IMMEDIATE dispatch counts: a queued acceptance is not a receipt for
      // this bubble.
      if (receipt.status === 'dispatched') dispatch(confirmOptimisticSend({ slot: slotKey, sendId }))
      ...
      >
        ...
        <PendingQuestionCard
          ...
          /* doSend() reads the composer state, so the fallback sends directly
             ...
             user's answer outright; on refusal, drop, transport failure, or
