  ...
  return <div data-role="assistant" className="group/msg">
    ...
    {!isStreaming && showFooter && (<>
      <div className={ACTIONS_REVEAL_CLS}>
        ...
        {onFork && forkIndex !== undefined && <button className={ROW_ACTION_CLS} disabled={busyAction !== null} data-testid="fork-from-here" title={forkLabel} aria-label={forkLabel} onClick={async () => { setBusyAction('fork'); try { await (forkMessageId ? onFork(forkIndex, forkMessageId) : onFork(forkIndex)) } finally { setBusyAction(null) } }}>{busyAction === 'fork' ? <Loader2 size={14} className="animate-spin" /> : <GitFork size={14} />}</button>}
        {onPlanFromHere && forkIndex !== undefined && <button className={ROW_ACTION_CLS} disabled={busyAction !== null} data-testid="plan-from-here" title={planLabel} aria-label={planLabel} onClick={async () => { setBusyAction('plan'); try { await (forkMessageId ? onPlanFromHere(forkIndex, forkMessageId) : onPlanFromHere(forkIndex)) } finally { setBusyAction(null) } }}>{busyAction === 'plan' ? <Loader2 size={14} className="animate-spin" /> : <ClipboardList size={14} />}</button>}
