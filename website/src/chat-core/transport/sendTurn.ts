 ...
 * layer adds the outcomes a parse cannot see -- the busy-race drop, the abort
 * deadline, the transport reject -- and splits acceptance by what it means for
 ...
 *                        bubble (#4131): no `chat_message` echo is coming for a
 ...
 * - `response-late`   -- the abort deadline fired. Same posture as `unknown`:
 *                        the request was received, the output arrives over the
 *                        socket, act on nothing.
...
export async function sendTurn(opts: SendTurnOptions): Promise<SendReceipt> {
  ...
  try {
    ...
    if (outcome === 'unknown') return { status: 'unknown', body }
