...
import SessionsPage, { recencyGroup, slotTsMs } from '../pages/SessionsPage'
...
function storeWith(slots: ChatSlot[], unread: string[] = []) {
  ...
  return createTestStore({
    ...state,
    dashboard: { ...state.dashboard, slots, unreadSlots: unread },
  })
}
...
describe('slotTsMs', () => {
  it('prefers last_activity_ts, falls back to last_ts then created, 0 when unparseable', () => {
    const t1 = new Date(2026, 7, 28, 10).toISOString()
    const t2 = new Date(2026, 7, 27, 10).toISOString()
    expect(slotTsMs(slot('a', 'a', 0, { last_activity_ts: t1, last_ts: t2 }))).toBe(Date.parse(t1))
    expect(slotTsMs(slot('b', 'b', Date.parse(t2)))).toBe(Date.parse(t2))
    expect(slotTsMs({ key: 'c', messages: 0, running: false } as ChatSlot)).toBe(0)
  })
})
...
describe('SessionsPage', () => {
  ...
  it('renders the page header title and marks the active chip with aria-pressed', async () => {
    ...
    expect(await screen.findByTestId('page-title')).toHaveTextContent('Sessions')
