 ...
 *  (user, nudge, subagent) plus injected prompts (cron, note, synthesis). */
function opensTurn(m: ChatMessage): boolean {
  return m.role === 'inject' || TURN_OPENER_ROLES.has(m.role)
}
...
export function useChatNavigation(
  ...
  const sections = useMemo(() => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      result.push({
        id: typeof explicitId === 'string' && explicitId ? explicitId : `turn-${i}`,
        label,
        prompt,
        response,
        msgIdx: i,
