...
export function appNavTarget(app: AppNavRecord): AppNavTarget | null {
  ...
  return {
    ...
    iconUrl: installedArt(app.manifest?.iconPath, app.name)
      || installedArt(app.manifest?.iconUrl, app.name),
    iconUrlDark: installedArt(app.manifest?.iconPathDark, app.name)
      || installedArt(app.manifest?.iconUrlDark, app.name),
    iconName: page.icon || '',
