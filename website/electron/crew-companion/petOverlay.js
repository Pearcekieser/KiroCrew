...
function createBrainWindow() {
  ...
  const recreate = () => {
    ...
    // Do NOT clear currentBubble: the replacement brain cannot reproduce a socket
    // completion/approval (watchSessions sees only future frames) or an already-
    // committed reminder, so clearing would silently drop the visible notification.
    // The bubble stays until the new brain reports fresh state.
    ensureBrainWindow();
  };
...
function registerOverlayIpc() {
  ...
  ipcMain.on("crew-companion:bubble-state", (event, bubble) => {
    const win = BrowserWindow.fromWebContents(event.sender);
