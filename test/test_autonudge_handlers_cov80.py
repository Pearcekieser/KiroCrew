...
@pytest.mark.asyncio
async def test_session_close_record_cannot_be_replaced_or_restarted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    loop = _monitor_loop()
    assert loop.monitor is not None
    ...
    replacement = await h.api_monitor_create(
        _mk(
            "POST",
            "/api/monitors",
            body={
    ...
    assert _body(replacement)["code"] == "monitor_not_restartable"
