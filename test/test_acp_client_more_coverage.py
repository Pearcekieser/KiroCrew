...
class TestEnsureReady:
    ...
    @pytest.mark.asyncio
    async def test_sandbox_preflight_binds_an_owner_the_deadline_gives_up(self, monkeypatch):
        """The preflight worker's resolver requests carry an owner, like the read
        gate's: a worker promoted onto the helper's lock AFTER this preflight's
        deadline must find its owner gone and return rather than go in flight
        ownerless (two bunched preflights on a wedged mount would otherwise pin
        the lock gateway-wide)."""
        import threading

        from kiro_crew.security import pathres_client

        monkeypatch.setattr(acp_client, "_SANDBOX_PREFLIGHT_TIMEOUT", 0.05)
        monkeypatch.setattr(
            acp_client.pathres_client.helper(), "abort_if_inflight", lambda tid: True
        )
        release = threading.Event()
        seen: list[threading.Event | None] = []

        def _stalled(backend, mode):
            seen.append(getattr(pathres_client._OWNER, "gave_up", None))
            release.wait(5.0)
            return ()
        ...
        try:
            ...
            release.set()
        assert len(seen) == 1 and seen[0] is not None, "the worker ran with no owner bound"
        assert seen[0].is_set(), "the deadline did not give the owner up"
