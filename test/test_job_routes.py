...
@pytest.mark.asyncio
async def test_start_with_non_object_params_is_400_invalid_params(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, sdk: JobSDK
) -> None:
    _setup_guards(tmp_path, monkeypatch)
    sdk.register("k", lambda handle, **p: None)
    async with TestClient(TestServer(_make_app())) as client:
        resp = await client.post(f"{_base()}/k/start", json={"params": [1, 2, 3]})
        assert resp.status == 400
        assert (await resp.json())["code"] == "invalid_params"
