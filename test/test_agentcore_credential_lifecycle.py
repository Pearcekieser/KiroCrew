...
async def test_gateway_recycle_releases_held_semaphore(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        await mgr.get_or_create("dash:recycle")
        mgr.release("dash:recycle")
        sess = mgr._sessions["dash:recycle"]
...
async def test_gateway_apply_error_releases_held_semaphore(
    ...
    mgr = SessionManager(KiroCrewConfig(), provider_factory=factory)
    try:
        await mgr.get_or_create("dash:apply-err")
        mgr.release("dash:apply-err")
        sess = mgr._sessions["dash:apply-err"]
...
async def test_staged_apply_restages_bind_after_recycle(
    ...
    gw.stage_session_gateway("dash:1", "dashboard", "alice", agent="research")
    ...
    assert await gw.apply_staged_session_gateway(None, "dash:1") is True
    restaged = gw._staged_for("dash:1")
    assert restaged is not None and restaged[3] == "research"
    assert gw.take_staged_gateway("dash:1") == ("dashboard", "alice")
...
async def test_install_staged_sidecar_does_not_take(
    ...
    gw.stage_session_gateway("dash:1", "discord", "alice", agent="research")
    ...
    monkeypatch.setattr(gw, "_apply_session_gateway", _apply)
    await gw.install_staged_gateway_sidecar("dash:1")
    assert applied == [("discord", "alice", None, "research")]
    assert gw.peek_staged_gateway("dash:1") == ("discord", "alice")
    assert gw.take_staged_gateway("dash:1") == ("discord", "alice")
...
@pytest.mark.parametrize(
    ...
    [
        ("dash:stale-new", None),
        ("dash:stale-load", "sid-resume-stale"),
    ],
