...
#
# _RESPAWN_TIMEOUT stays BELOW the production give-up it races
# (``subagent._RECOVERY_SLOT_WAIT_SECS`` = 60.0). If the poll ever outlived that, the
# code would raise "no free slot for recovery respawn", ``task2`` would never appear,
# and the failure would read as "recovery never happened" instead of "the test
# outwaited the code".
#
