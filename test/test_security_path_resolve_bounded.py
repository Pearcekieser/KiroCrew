...
def test_root_anchors_name_each_spelling_once_in_the_request(
    _real_helper, monkeypatch, tmp_path
) -> None:
    """Several env vars routinely name the SAME directory. One request is not the
    whole saving: the repeat must be gone BEFORE it goes out, because the helper
    resolves the list it is handed and only the answer dict collapses duplicates."""
    ...
    sent = requests[0]
    assert sent.count(shared) == 1, sent
    assert len(sent) == len(set(sent)), sent
    # The collapse must not cost either root its answer.
    assert roots.crew_home == os.path.realpath(shared)
    assert roots.kiro_home == os.path.realpath(shared)
...
def test_a_stall_reads_as_unverifiable_and_a_healthy_path_as_clear(
    ...
    try:
        ...
        assert security.is_unverifiable_path_refusal(refusal)
...
def test_an_empty_spelling_never_reaches_the_resolver(_real_helper, monkeypatch) -> None:
