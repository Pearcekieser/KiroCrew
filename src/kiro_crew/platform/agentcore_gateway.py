...
# token. ``subagent:``, ``hook:``, ``channel:``, and workflow keys are
# named because bind/inject must treat a child, signed webhook, crew
# channel-agent, or workflow run the same as cron — not a human at the
# keyboard. ``channel:`` is ``run_channel_agent``
# (``channel:{id}:{agent}``), not a human ChannelTurn (those use
# ``slack:`` / ``discord:`` / …). ``wf:`` does not prefix ``wf-pool:``
# / ``wf-author:`` / ``wf-unpooled:`` (third character differs), so
# those stay listed. Overflow workers are ``wf-unpooled:{run}:{i}``.
...
def is_unattended_session(session_key: str) -> bool:
    """True when *session_key* names cron / TaskRunner / heartbeat, not a human."""
    if not session_key:
        return False
    if session_key in UNATTENDED_SESSION_KEYS:
        return True
    return session_key.startswith(UNATTENDED_SESSION_PREFIXES)
    ...
    if data is None:
        if is_unattended_session(session_key) and not _unattended_user_permitted():
            ...
            if _current_posture() == "login":
                ...
                return []
            # Workload user/OBO without a vaulted owner token: retract
            # Gateway the same way attach does, and leave a SEL trail.
...
async def attach_gateway_inbound(principal: SessionPrincipal, *, agent: str = "") -> Path | None:
    ...
    profile permits it.
    """
    if not _identity_on(principal.session_key, agent=agent):
...
async def prepare_session_gateway(
    ...
    *,
    ...
    ``session/new`` cannot read a leftover bearer, then applies again after
    the same-key winner holds the per-session semaphore (recycle if the
    fingerprint changed). Pre-acquire attach/recycle is what let concurrent
