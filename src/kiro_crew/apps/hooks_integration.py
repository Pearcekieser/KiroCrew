...
async def on_app_disable(
    ...
    *,
    ...
    # Clean up durable job runs owned by this app, mirroring the cron contract:
    # idempotent, and a failure is REPORTED rather than crashing the disable.
    # Signalling is all the SDK can do about the WORK -- a runner that never
    # polls its handle within the deadline is reported -- and the RECORDS stay
    # deleted: the SDK marks every live handle discarded under the same lock its
    # guarded writer takes, so a worker returning mid-cleanup cannot write its
    # record back, and a partial delete is reported rather than read as clean.
    if permissions.get("jobs"):
        job_sdk = get_sdk(app_name)
        if job_sdk is not None:
            try:
                cleanup = await job_sdk.remove_all_async()
                if not cleanup.is_clean:
                    # Reported, not swallowed: a cleanup that left records behind
                    # OR left app code executing must not read as clean. Same rule
                    # the cron cleanup above follows.
                    parts = []
                    if cleanup.failed:
                    ...
                    result["job_cleanup"] = f"partial: removed {cleanup.removed}, " + "; ".join(
                        parts
                    )
                    sel().log_api_access(
                        caller="gateway",
                ...
                logger.warning(
                    "App %s: job cleanup could not complete on disable: %s", app_name, exc
                )
    ...
    # Drop the lookup entry UNCONDITIONALLY, outside the grant check above: if
    # the manifest's `jobs` grant was revoked before the disable, that check is
    # False and a conditional forget would leave the SDK published for the rest
    # of the gateway's life. The route guard re-reads the manifest so it would
    # refuse anyway, but a revoked capability must not survive in the registry.
    forget_sdk(app_name)
