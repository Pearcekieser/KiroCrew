...
def _public_view(run: JobRun) -> dict[str, Any]:
    ...
    ``params`` is withheld on purpose: a start call's arguments can carry
    something the caller would not want echoed back into a page (a key, a path,
    an account id), and no client needs them to render a run. ``origin`` and
    ``pid`` are host facts with no client meaning.
    """
    return {
        "run_id": run.run_id,
        "kind": run.kind,
...
async def _handle_start(request: web.Request, app_name: str, sdk: JobSDK) -> web.StreamResponse:
    ...
    if params is not None and not isinstance(params, dict):
        return web.json_response(
            {"error": "params must be an object", "code": "invalid_params"}, status=400
        )
    ...
    try:
        run_id = await sdk.start_async(kind, params=params, dedupe_key=dedupe_key)
        ...
        return web.json_response({"error": str(exc), "code": "unknown_job_kind"}, status=404)
