...
def test_job_registry_start_returns_id_and_poll_returns_result() -> None:
    async def go() -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
...
def test_job_status_unknown_id_is_404() -> None:
    resp = asyncio.run(routes._handle_job_status(_QReq("does-not-exist")))  # type: ignore[arg-type]
    assert resp.status == 404
    assert isinstance(resp.body, bytes) and b"unknown_job" in resp.body


def test_discover_missing_field_is_400_synchronously() -> None:
    # Obvious bad input is rejected with 400 before any job starts.
    resp = asyncio.run(routes._handle_discover(_Req({"kind": "repo"})))  # type: ignore[arg-type]
    assert resp.status == 400
    assert isinstance(resp.body, bytes) and b"missing_field" in resp.body


def test_render_starts_a_job_then_poll_returns_the_screens(monkeypatch, tmp_path) -> None:  # type: ignore[no-untyped-def]
...
def test_render_starts_a_job_even_when_bad_input_would_fail_later(
