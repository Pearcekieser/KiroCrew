...
def _resolve_one(path: str, *, both: bool = True) -> list[str | None]:
    ...
    With ``both`` false only ``realpath`` is computed and the second slot is
    ``None``: the anchors want one spelling, and on Windows each spelling costs a
    handle open per path component, so an anchor batch asking for both paid
    twice for a value it discarded.
    """
    try:
...
def serve(stdin, stdout) -> None:
    ...
    for raw in stdin:
        try:
            ...
            paths = request["p"]
            both = request.get("m") != "real"
            if isinstance(paths, str):
                answer: object = _resolve_one(paths, both=both)
            elif isinstance(paths, list) and all(isinstance(p, str) for p in paths):
                answer = [_resolve_one(p, both=both) for p in paths]
            else:
                return
