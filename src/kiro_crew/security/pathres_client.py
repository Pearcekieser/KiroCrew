...
#: Per worker thread: the deadline owner's "gave up" flag for the resolution the
#: thread is currently running (see :func:`owner`). Bound for the duration of one
#: resolution, never keyed by thread id: pool threads are long-lived and a mark
#: that outlived its resolution would refuse the thread's NEXT, unrelated request.
_OWNER = threading.local()


@contextlib.contextmanager
def owner(gave_up: threading.Event) -> Iterator[None]:
    """Bind *gave_up* to this thread while it runs one resolution.

    The deadline owner (``paths._run_resolution_bounded``) sets the event when it
    stops waiting. A worker still QUEUED behind another request at that moment
    would otherwise be promoted later and send its path -- possibly the same wedged
    one -- with no owner left to ``abort_if_inflight`` it; bound here, the request
    returns ``None`` at promotion instead. A request already in flight is
    unaffected (its answer is simply discarded).
    """
    previous = getattr(_OWNER, "gave_up", None)
    _OWNER.gave_up = gave_up
    try:
        yield
    finally:
        _OWNER.gave_up = previous


def _inprocess(path: str, *, both: bool = True) -> tuple[str | None, str | None]:
    """The helper's own resolution, run in this interpreter when no helper can."""
    real, resolved = _resolve_one(path, both=both)
...
class ResolverHelper:
    ...
    def resolve_many(
        self, paths: list[str], *, realpath_only: bool = False
        ...
        if not paths:
            return []
        if self._latched():
            return [_inprocess(p, both=not realpath_only) for p in paths]
        return self._request(paths, paths, both=not realpath_only)

    def _request(
        self, payload: str | list[str], paths: list[str], *, both: bool = True
    ) -> list[tuple[str | None, str | None]] | None:
        tid = threading.get_native_id()
        self._phase[tid] = "queued"
        try:
            with self._request_lock:
                return self._request_locked(payload, paths, tid, both=both)
        finally:
            self._phase.pop(tid, None)

    def _request_locked(
        self, payload: str | list[str], paths: list[str], tid: int, *, both: bool = True
    ) -> list[tuple[str | None, str | None]] | None:
        owner_gave_up = getattr(_OWNER, "gave_up", None)
        if owner_gave_up is not None and owner_gave_up.is_set():
            # Promoted from the queue after the deadline owner refused and moved on
            # (see ``owner``): nothing waits for the answer, and sending would put a
            # possibly wedged path in flight with no owner left to abort it. Two
            # of those would pin both pool workers and the gate would refuse
            # everything until restart.
            return None
        self._phase[tid] = "spawning"
        proc = self._spawn()
        if proc is None:
            return [_inprocess(p, both=both) for p in paths]
        ...
        try:
            request: dict[str, object] = {"i": seq, "p": payload}
            if not both:
                request["m"] = "real"
