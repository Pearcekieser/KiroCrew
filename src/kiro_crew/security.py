...
def is_sensitive_bash_command(command: str) -> str | None:
    ...
    if _names_unreplaceable_container_raw(command):
        return (
...
def _build_container_regex() -> "re.Pattern[str]":
    ...
    dirs_pattern = "|".join(re.escape(d) for d in dirs)
...
def _substitution_could_name_container(text: str) -> bool:
    ...
    if not _OUTPUT_SUBSTITUTION_RE.search(text):
        return False
    targets = _home_dir_targets(sorted(_UNREPLACEABLE_CONTAINER_DIRS))
    ...
    masked = _OUTPUT_SUBSTITUTION_RE.sub("*", text)
    examined = 0
...
def _names_unreplaceable_container_raw(command: str) -> bool:
    ...
    * and `cd` is a shell builtin that a function or alias can shadow, so even a
    ...
    if _get_container_re().search(_empty_expansion_reading(decoded)) is not None:
        return True
    return _substitution_could_name_container(decoded)
...
def _glob_modes(command: str) -> "dict[str, bool]":
    ...
    """
    return {
        "dotglob": bool(_DOTGLOB_ENABLED_RE.search(command)),
        "globstar": bool(_GLOBSTAR_ENABLED_RE.search(command)),
        "extglob": bool(_EXTGLOB_ENABLED_RE.search(command)),
    }
...
def _glob_could_name(
    ...
    * it ignores the dotfile rule, so `~/*` matched `~/.kiro` even though bash without
    ...
    if extglob:
        ...
        token = _EXTGLOB_GROUP_RE.sub(".*", token)
    ...
    for source in (_mask_substitutions(command), command):
        ...
        for token in full_tokens:
            ...
            for cand in _path_candidates(token, budget):
                ...
                if is_unreplaceable_container(cand):
                    return (
                ...
                for cand in _path_candidates(token, budget):
                    ...
                    if not os.path.isabs(cand) and not cand.startswith("~"):
                        for base in seen_bases:
                            ...
                            if is_unreplaceable_container(candidate):
                                return (
